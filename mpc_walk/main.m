...
%%
sim_k = 1;
for sim_t = 0:ctr_p.dt_mpc:2
    %fprintf('sim time %f of %f \n', sim_t, 6);

    % update global clock
    ctr_p.t_gloal_n = sim_t;
    % current state
    x_t = x_arr(:,sim_k);
    % get ref traj
    [ref_traj_v, ctr_p] =...
        ref_traj_planner(x_t, vel_tar, world_p, body_p, ctr_p, path);
    ...
    x_next(1:6) = x_next(1:6) + ctr_p.dt_mpc*vel_tar;
    x_arr = [x_arr, x_next];

    sim_k = sim_k+1;
        %%%%%
    ctr_p.counter_gait = ctr_p.counter_gait + ctr_p.dt_mpc/ctr_p.dt_gait;
end
...
for sim_t = 0:ctr_p.dt_mpc:4
    ...
    [x_sol, f_sol, fp_l_sol, fp_g_sol] =...
        unpacks_sol(sol, body_p, ctr_p, path);
    ...
    % sim next timestep
    tspan = [sim_t, sim_t+ctr_p.dt_mpc];
    ...
    %x_next = x_out(:,end);
    x_next = x_t + dyn_f(x_t,f_optimal,fp_optimal)*ctr_p.dt_mpc;
    x_next = full(x_next);
...
rbt_anime(x_arr,f_arr,fpp_g_arr,[], ctr_p.t_mpc_horizon, ctr_p.mpc_horizon_steps);
%%
clf;
