...
function [mpc_v, mpc_c, mpc_p] = form_mpc_prob(world_p, body_p, ctr_p, obst, dyn_f, path)
...
% obst constraint
mpc_c.ineq_con_obst_xy = SX.zeros(obst.num*N,1); % obst distance range, >0
mpc_c.ineq_con_obst_z = SX.zeros(N,1); % obst distance range z, >0

mpc_c.ineq_con_dim = 4*6*N + 4*4*N + N + 4*N + obst.num*N + N; % dim for ieq constraints, obst dist dim added
...
for k = 1:N
    ...
    %z obst
    mpc_c.ineq_con_obst_z(k) =...
        -1 * (sqrt((obst.vood_x_pos - x_t(4))^2 + x_t(6)^2) - obst.h);
...
mpc_c.constraint_arr = [mpc_c.eq_con_init_state; mpc_c.eq_con_dynamic; mpc_c.eq_con_foot_contact_ground; mpc_c.eq_con_foot_non_slip;...
                        mpc_c.ineq_con_foot_range; mpc_c.ineq_con_foot_friction; mpc_c.ineq_con_zforce_dir; mpc_c.ineq_con_zforce_range; mpc_c.ineq_con_obst_xy; mpc_c.ineq_con_obst_z];
