...
% state lower & upper boundary, these are for values being optimized
% state
...
ub_state_arr = repmat(ub_state,ctr_p.mpc_horizon_steps+1,1);
lb_state_arr = repmat(lb_state,ctr_p.mpc_horizon_steps+1,1);
...
ub_f_arr = repmat(ub_f,ctr_p.mpc_horizon_steps,1);
lb_f_arr = repmat(lb_f,ctr_p.mpc_horizon_steps,1);

% foot pos, deleted
% ub_fp = repmat([0.4;0.4;inf],4,1);
% lb_fp = repmat([-0.4;-0.4;-inf],4,1);
% ub_fp_arr = repmat(ub_fp,ctr_p.mpc_horizon_steps,1);
% lb_fp_arr = repmat(lb_fp,ctr_p.mpc_horizon_steps,1);
